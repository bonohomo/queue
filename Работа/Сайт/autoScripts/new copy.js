  ...
  for (
    ...
    try {
      ...
      for (
        ...
        console.log(
          `%c[${i + 1}/${links.length}] PRICE ATTEMPT ${attempt}/${MAX_ATTEMPTS}`,
          "color:#7950f2;font-weight:bold"
        );


        // ------------------------------------------------------
        // Каждый раз получаем input ЗАНОВО
        // ------------------------------------------------------
        ...
        if (!actualInput) {
          ...
          throw new Error(
            "Поле цены исчезло перед попыткой ввода"
          );
        ...
        console.log(
          `💰 Перед попыткой: "${actualInput.value}"`
        );
        ...
        if (!actualInput) {
          ...
          console.warn(
            `⚠️ После попытки ${attempt} поле не найдено`
          );

        } else {

          const numericValue =
            actualInput.value.replace(
              /[^\d]/g,
              ""
            );
          ...
          console.log(
            `💰 После попытки ${attempt}: "${actualInput.value}"`
          );
          ...
          if (numericValue === PRICE) {
            ...
            console.log(
              `%c✓ Цена установлена с попытки ${attempt}`,
              "color:#2f9e44;font-weight:bold"
            );
        ...
        console.warn(
          `⚠️ Попытка ${attempt}/${MAX_ATTEMPTS} не сработала`
        );
        ...
        if (attempt < MAX_ATTEMPTS) {
          ...
          console.log(
            "⏳ Жду 2 секунды перед повторной попыткой..."
          );
      ...
      if (!priceChanged) {
        ...
        throw new Error(
          `Не удалось установить ${PRICE} после ${MAX_ATTEMPTS} попыток`
        );
